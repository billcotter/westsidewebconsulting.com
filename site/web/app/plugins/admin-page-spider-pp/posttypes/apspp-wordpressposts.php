...
//check there is more than 1 post so that the user actually blogs.
	...
	//Adds a "Posts" menu to adminbar
		$apspider_which_option_is_selected = get_site_option('apspp_radio_viewposts');
		if ( ! empty ( $apspider_which_option_is_selected[0] )){
			if ( $apspider_which_option_is_selected[0] == 'option1'){
				add_action( 'admin_bar_menu', 'apspp_wp_view_posts', 996 );
				add_action('apspp_extra_styles', 'apspp_posts_extra_styles', 10);
			}
		}
	}
}

function apspp_wp_view_posts($wp_admin_bar) {
...
function apspp_posts_extra_styles(){
