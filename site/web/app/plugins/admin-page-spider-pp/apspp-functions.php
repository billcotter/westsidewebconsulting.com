...
function apspp_add_custom_admin_styles() {
...
do_action('apspp_extra_styles');
echo '</style>';

echo'<script type="text/javascript">
	jQuery(document).ready(function(){
		jQuery("input.apspider_search").on("keyup", function(){
			searchFilter = jQuery(this).val().toLowerCase();
			jQuery(this).parentsUntil(".apspider_menu_class").find("a").each(function(){

				thisItem = jQuery(this).text().toLowerCase();
				if (thisItem.includes(searchFilter))
				{
					if (jQuery(this).has("[title]") && !jQuery(this).parent().hasClass("apspider_highlighted"))
					{
...
add_action('admin_head', 'apspp_add_custom_admin_styles', 11);
add_action('wp_head', 'apspp_add_custom_admin_styles', 11);
...
if (count($this_list) > 0)
{
	...
	//Create 'search' Menu Node
		$args = array(
			'id'    => 'viewapspider_s' . $unique_slug,
			'title' => '<input class="apspider_search" placeholder="Type to filter" autocomplete="off" type="text">',
			'href'  => '',
			'parent' => 'viewapspider_'. $unique_slug,
			'meta' => array( 'class' => 'apspider_highlighted_search')
			);
		$wp_admin_bar->add_node( $args );
	}
	else
	...
	foreach( $this_list as $post )
	{
		...
		if ($post->post_status == 'draft' && $post->post_type == 'fl-theme-layout'){
			$suffix .= ' <span style="font-size: 10px; color: #888888; position: absolute; right: 20px;">(Draft Layout)</span>';
		}
		...
		else if ($post->post_type == 'fl-theme-layout'){
			$suffix .= ' <span style="font-size: 10px; color: #888888; position: absolute; right: 20px;">(Layout)</span>';
		}
		...
		if ($post->post_type != 'acf' && $post->post_type != 'acf-field-group')
		{


			if($post->post_type == 'fl-theme-layout')
			{
				//Create Basic Edit Link
				$args = array(
				'id'    => $post->ID . $unique_slug. 'default_edit',
				'title' => '',
				'href'  => $admin_url . 'post.php?post=' . $post->ID . '&action=edit',
				'parent' => $post->ID . $unique_slug.'wppg',
				'meta'  => array( 'class' => 'apspider_highlighted_view icon2'. $extraclass, 'title' => __('Edit in Admin', 'admin_page_spider'))
				);
				$wp_admin_bar->add_node( $args );
			}
...
function edd_apspp_sync_license() {

	$license = trim( get_site_option( 'edd_apspp_license_key' ) );

	$thisurl = network_site_url();
	...
	if ( $license) {
		...
		$api_params = array(
			...
			'license' 	=> $license,
			'item_name' => urlencode( EDD_APSPP_ITEM_NAME ),
			'url'       => $thisurl
			);
...
function apspp_license_sync_periodic()
{
	...
	$proceed = false;
	if(strtotime(get_site_option('edd_apspp_check_transient')) < strtotime('-7 days'))
	{
		// this is older than specified days
		$todaysdate = date("Y-m-d H:i:s");

		// Check remote status
		$license_data = edd_apspp_sync_license();

		if (isset($license_data))
		{
			if ($license_data->license == 'invalid' OR
				$license_data->license == 'expired' OR
				$license_data->license == 'disabled' OR
				$license_data->license == 'inactive' OR
				$license_data->license == 'site_inactive')
			{
				apspp_deleteall_settings();
			}
		}
		update_site_option('edd_apspp_check_transient',$todaysdate);
	}

}

add_action( 'current_screen', 'apspp_license_sync_periodic' );
