<?php
/**
 * Builds the Child Themer Options function.
 *
 * @package Child Themer
 */
 
/**
 * Get the latest child_themer_settings array from the database
 * and then cache it, if not otherwise specified, so specific
 * Child Themer Settings values (or the entire array) can be efficiently accessed.
 *
 * @since 1.0.0
 * @return either the entire child_themer_settings array or a specific key/value.
 */
function child_themer_get_settings( $key, $args = false ) {
	...
	if ( $args ) {
		...
		// then update the $options_cache variable with the latest copy of the child_themer_settings array.
		if ( empty( $options_cache ) || !$args['cached'] )
			$options_cache = get_option( 'child_themer_settings' );

		// If the $args['array'] key is not false then return the entire
		// child_themer_settings array through the $options_cache variable.
		if ( $args['array'] ) {
	...
	// latest copy of the child_themer_settings array and set the $options_set variable to true.
	} else {
