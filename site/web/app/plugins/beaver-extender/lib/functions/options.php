<?php
/**
 * Builds the Beaver Extender Options functions.
 *
 * @package Extender
 */
 
/**
 * Get the latest beaver_extender_settings array from the database
 * and then cache it, if not otherwise specified, so specific
 * Beaver Extender Settings values (or the entire array) can be efficiently accessed.
 *
 * @since 1.0.0
 * @return either the entire beaver_extender_settings array or a specific key/value.
 */
function beaver_extender_get_settings( $key, $args = false ) {
	...
	if ( $args ) {
		...
		// then update the $options_cache variable with the latest copy of the beaver_extender_settings array.
		if ( empty( $options_cache ) || !$args['cached'] )
			$options_cache = get_option( 'beaver_extender_settings' );

		// If the $args['array'] key is not false then return the entire
		// beaver_extender_settings array through the $options_cache variable.
		if ( $args['array'] ) {
	...
	// latest copy of the beaver_extender_settings array and set the $options_set variable to true.
	} else {
...
/**
 * Get the latest beaver_extender_custom_css array from the database
 * and then cache it, if not otherwise specified, so specific
 * Custom Options values (or the entire array) can be efficiently accessed.
 *
 * @since 1.0.0
 * @return either the entire beaver_extender_custom_css array or a specific key/value.
 */
function beaver_extender_get_custom_css( $key, $args = false ) {
	...
	if ( $args ) {
		...
		if ( empty( $options_cache ) || !$args['cached'] )
			$options_cache = get_option( 'beaver_extender_custom_css' );
