 ...
 */
function beaver_extender_get_templates() {
	
	$custom_templates = get_option( 'beaver_extender_custom_templates' );
	...
	if ( ! empty( $custom_templates ) ) {
		...
		$custom_templates = beaver_extender_array_sort( $custom_templates, 'template_file_name' );		
 ...
 * @since 1.0.0
 */
function beaver_extender_update_templates( $file_names = '', $template_names = '', $template_types = '', $template_post_types = '', $textareas = '' ) {
	
	$beaver_extender_templates_array = get_option( 'beaver_extender_custom_templates' );
	...
	if ( ! empty( $these_templates ) ) {
		...
		foreach( $these_templates as $this_template ) {
			...
			if ( ! empty( $template_id ) ) {
				...
				$merged_page_template = array_merge( $beaver_extender_templates, $new_values );
				update_option( 'beaver_extender_custom_templates', $merged_page_template );
				
			}
			
		}
		foreach( $beaver_extender_templates_array as $key1 => $value1 ) {
			...
			if ( in_array( $value1['template_file_name'], $updated_template_file_name_array ) ) {
				...
				foreach( $updated_template_type_array as $key2 => $value2 ) {
					...
					if ( !in_array( $value1['template_type'], $updated_template_file_name_array ) && file_exists( beaver_extender_get_custom_template_paths( $template_file_name_value, $value1['template_type'] ) ) )
						unlink( beaver_extender_get_custom_template_paths( $template_file_name_value, $value1['template_type'] ) );
 ...
 * was created inside the root folder of the active Beaver Builder Child Theme.
 *
 * @since 1.0.0
 */
add_action( 'wp_ajax_beaver_extender_template_delete', 'beaver_extender_delete_template' );
function beaver_extender_delete_template() {
	...
	foreach( $beaver_extender_templates as $key => $value ) {
		...
		if ( $template_file_name == $template_file_name_value ) {
			...
			unset( $beaver_extender_templates[$key] );
			unlink( beaver_extender_get_custom_template_paths( $template_file_name_value, $value['template_type'] ) );
			
		}
		
	}

	update_option( 'beaver_extender_custom_templates', $beaver_extender_templates );
 ...
 * @since 1.0.0
 */
function beaver_extender_reset_delete_template() {
